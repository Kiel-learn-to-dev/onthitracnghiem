...
type PublishedExam = { id: string; title: string; questionCount: number };
...
async function startAttempt(payload: Record<string, string> = {}, button?: HTMLButtonElement): Promise<void> {
...
async function setupHome(): Promise<void> {
  const randomButton = document.querySelector<HTMLButtonElement>("#start-exam");
  ...
  if (!container) return;
  try {
    const published = await request<{ data: PublishedExam[] }>("/api/exams/published");
    ...
    if (randomPublishedButton) randomPublishedButton.disabled = !published.data.length;
    if (!published.data.length) setText("#home-status", "Chưa có đề sẵn; hãy dùng đề ngẫu nhiên.");
  } catch (error) {
...
async function setupExam(): Promise<void> {
  ...
  try {
    ...
    const updateCountdown = () => {
      ...
      if (remaining.expired) void submit();
    };
